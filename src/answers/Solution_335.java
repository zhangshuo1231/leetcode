...
public class Solution_335 {
    public boolean isSelfCrossing(int[] distance) {
        ...
        for(int i = 3; i < l; i++){
            ...
            if(i >= 5)
            {
                if(distance[i-2] - distance[i-4] >= 0 && distance[i] >= distance[i-2] - distance[i-4] && distance[i-1] >= distance[i-3] - distance[i-5] && distance[i-1] <= distance[i-3]) return true;  // Sixth line crosses first line and onward
            }
        }
        return false;
    }
