...
class Solution_698 {
    ...
    private boolean backtrack(int[] arr, int index, int numSubsetsMade, int currSum, int k, int targetSum, char[] taken, HashMap<String, Boolean> memo) {
        int n = arr.length;
        // We made k - 1 subsets with target sum and last subset will also have target sum.
        if (numSubsetsMade == k - 1) {
            return true;
        }
        ...
        // Increment numSubsetsMade and reset current sum.
        if (currSum == targetSum) {
            boolean ans = backtrack(arr, 0, numSubsetsMade + 1, 0, k, targetSum, taken, memo);
        ...
        for (int j = index; j < n; ++j) {
            if (taken[j] == '0') {
                ...
                if (backtrack(arr, j + 1, numSubsetsMade, currSum + arr[j], k, targetSum, taken, memo)) {
                    return true;
                }
